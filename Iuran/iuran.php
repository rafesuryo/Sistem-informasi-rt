		...
		<form action="" method="post" enctype="multipart/form-data">
			<table class="table">
				...
				<tr>
					...
					<td>:
					<td width="683"><input style="width: 250px;"  name="bukti_struk" class="form-control" type="file" id="bukti_struk" value="<?php echo $bukti_struk; ?>" size="25" />
					</td>
				</tr>
...
<?php } ?>
				<tr>
					...
					<td colspan="2">
						...
						<input name="id_iuran0" type="hidden" id="id_iuran0" value="<?php echo $id_iuran0; ?>" />
...
+ Tambah Data</button></a>
	...
	foreach ($arrc as $dc) {
		$status = $dc["status"];
		$sql = "select * from `$tbiuran` where  `status`='$status' order by `id_iuran` desc";
				$jum = getJum($conn, $sql);
	?>
		<h3>Iuran Warga Status <?php echo $status. ": $jum Data "; ?></h3>
		<div>
			Data Iuran Warga <?php echo $status;?>| <img src='ypathicon/print.png' title='PRINT' OnClick="PRINT('<?php echo $status; ?>')"> |
			<table class="table table-bordered">
				...
				<td><div align='center'>
<a href='?mnu=iuran&pro=ubah&kode=$id_iuran&informasi=$informasi'><img src='ypathicon/ub.png' title='ubah'></a>
<a href='?mnu=iuran&pro=hapus&kode=$id_iuran&informasi=$informasi'><img src='ypathicon/ha.png' title='hapus' 
onClick='return confirm(\"Apakah Anda benar-benar akan menghapus $informasi pada data warga ?..\")'></a></div></td>
				</tr>";
						$no++;
					} //for dalam
				} //if
...
?>
		...
		if (isset($_POST["Simpan"])) {
			...
			if ($pro == "simpan") {
				...
				if ($simpan) {
					echo "<script>alert('Data $informasi berhasil disimpan !');document.location.href='?mnu=iuran';</script>";
				} else {
					echo "<script>alert('Data $informasi gagal disimpan...');document.location.href='?mnu=iuran';</script>";
				}
			} else {
				//update
			$id_iuran0 = strip_tags($_POST["id_iuran0"]);
				$sql = "update `$tbiuran` set 
					...
					`keterangan`='$keterangan'
					 where `id_iuran`='$id_iuran0'";
					 
				$ubah = process($conn, $sql);
				if ($ubah) {
					echo "<script>alert('Data $informasi berhasil diubah !');document.location.href='?mnu=iuran';</script>";
				} else {
					echo "<script>alert('Data $informasi gagal diubah...');document.location.href='?mnu=iuran';</script>";
				}
			} //else simpan
		}
		?>
		...
		if (isset($_GET["pro"]) && $_GET["pro"] == "hapus") {
			$id_iuran = $_GET["kode"];
			$nama_iuran = $_GET["nama_iuran"];
			$sql = "delete from `$tbiuran` where `id_iuran`='$id_iuran'";
			$hapus = process($conn, $sql);
			if ($hapus) {
				echo "<script>alert('Data $informasi berhasil dihapus !');document.location.href='?mnu=iuran';</script>";
			} else {
				echo "<script>alert('Data $informasi gagal dihapus...');document.location.href='?mnu=iuran';</script>";
			}
		}
		?>