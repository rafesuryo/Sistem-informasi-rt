		...
		<form action="" method="post" enctype="multipart/form-data">
			<table class="table">
				...
				<tr>
					...
					<td>:
					<td width="683">
					<?php echo $nama_pengguna. " $id_pengguna"; ?>
					</td>
				</tr>
...
<div align="right">
<a href='?mnu=wiuran&pro=tambah'>
<button type='button' title='tambah' class='btn btn-success btn-sm btn-icon-text mr-1'>
+ Tambah Data</button></a>
</div><br>
<h5> Norek 1223456 bca / dana 08123456789 </h5>
	
	<br></br>
	<?php //tampilan tabel data
	$sqlc = "select distinct(`status`) from `$tbiuran` order by `status` asc";
	...
	foreach ($arrc as $dc) {
		...
		$sql = "select * from `$tbiuran` where  `status`='$status' order by `id_iuran` desc";
				$jum = getJum($conn, $sql);
	?>
		<h3>Status <?php echo $status. " : $jum Data "; ?>:</h3>
		<div>
			Data Iuran Warga | <img src='ypathicon/print.png' title='PRINT' OnClick="PRINT('<?php echo $status; ?>')"> |
