...
<?php
$sql = "select `status` from `$tbproposal` where `status` like '%Proses%'";
$jum1 = getJum($conn, $sql)+0;
 $sql = "select `status` from `$tbproposal` where `status` like '%Disetujui%'";
$jum2 = getJum($conn, $sql)+0;
 $sql = "select `status` from `$tbproposal` where `status` like '%Ditolak%'";
  ...
  <head>
    ...
    <script type="text/javascript">
      ...
      function drawChart() {
        ...
        var options = {
          title: 'Laporan Status Proposal Proses s/d Disetujui '
        };
