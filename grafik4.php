...
<?php
$sql = "select `status` from `$tbpeserta` where `status` like '%Proses%'";
$jum1 = getJum($conn, $sql)+0;
 $sql = "select `status` from `$tbpeserta` where `status` like '%Disetujui%'";
$jum2 = getJum($conn, $sql)+0;
 $sql = "select `status` from `$tbpeserta` where `status` like '%Ditolak%'";
  ...
  <head>
    ...
    <script type="text/javascript">
      ...
      function drawChart() {
        ...
        var options = {
          title: 'Laporan Status Kepesertaan Proses s/d Disetujui '
        };
