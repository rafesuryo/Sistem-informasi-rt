...
+ Tambah Data</button></a>
	...
	$sqlc = "select distinct(`status`) from `$tbkegiatan` order by `status` asc";
	...
	foreach ($arrc as $dc) {
		$status = $dc["status"];
		$sql = "select * from `$tbkegiatan` where  `status`='$status' order by `id_kegiatan` asc";
		$jum = getJum($conn, $sql);
	?>
		<h3>Data Kegiatan : <?php echo $status . " $jum Data "; ?></h3>
		<div>
			Data Kegiatan | <img src='ypathicon/print.png' title='PRINT' OnClick="PRINT('<?php echo $status; ?>')"> |
			<table class="table table-bordered">
				<tr bgcolor="#cccccc">
					<th width="3%">No</td>
					<th width="7%">Gambar</td>
					<th width="35%">Detail Kegiatan</td>
					<th width="15%">Pengguna</td>
					<th width="10%">Ketegori</td>
				...
				<td><div align='center'>";
					...
					<img src='$YPATH/$gambar' width='40' height='40' /></a></div>";
						echo "</td> 	
				<td><b>$nama_kegiatan</b><br>
				    <b>Deskripsi:</b> $deskripsi</td>
				<td>$pengguna</td>
				<td>$kategori</td>
				<td>$tanggal-$jam</td>
				<td><div align='center'>
