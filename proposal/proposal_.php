	...
	<div>
		...
		<form action="" method="post" enctype="multipart/form-data">
			<table class="table">
				...
				<tr>
					<td><label for="id_pengguna">Pengguna</label>
					<td>:
					<td>
					<select class="form-control" name="id_pengguna" style="width: 350px;">
					<option value="selected" disabled>-- Pilih --</option>	
						<?php  
						$sql="select * from `$tbpengguna` where level='Ketua RT' or level='Karang Taruna' and status='Aktif'";
							$arr=getData($conn,$sql);
								foreach($arr as $d) {						
										$id_pengguna0=$d["id_pengguna"];
										$nama_pengguna=ucwords($d["nama_pengguna"]);
							echo "<option value='$id_pengguna0'"; if($id_pengguna0==$id_pengguna){echo"selected";} echo">$nama_pengguna ($id_pengguna0)</option>";
										}
										?>
						</select>
					</td>
				...
				<tr>
					<td><label for="jawaban">Alasan</label>
					<td>:
					<td>
...
+ Tambah Data</button></a>
	...
	foreach ($arrc as $dc) {
		...
		$sql = "select * from `$tbproposal` where  `status`='$status' order by `id_proposal` desc";
				$jum = getJum($conn, $sql);
	?>
		<h3>Data Proposal : <?php echo $status. " $jum Data "; ?></h3>
		<div>
			Data Proposal | <img src='ypathicon/print.png' title='PRINT' OnClick="PRINT('<?php echo $status; ?>')"> |
			<table class="table table-bordered">
				<tr bgcolor="#cccccc">
					<th width="3%">No</td>
					<th width="15%">Nama Proposal</td>
					<th width="15%">Pengguna</td>
					<th width="25%">Deskripsi</td>
					<th width="7%">File</td>
					<th width="7%">File Final</td>
					<th width="10%">Tanggal</td>
					<th width="5%">Alasan</td>
					<th width="5%">Menu</td>
				</tr>
				<?php
				$no=1;
				...
				<td>$pengguna</td>
				<td>$deskripsi</td>
				<td><a href='downloadgetfile.php?file=$file_proposal' target='_blank'>Download Porosal</a></td>	
				<td><a href='downloadgetfile.php?file=$file_jawaban' target='_blank'>Download Jawaban</a></td>
				<td>$tanggal - $jam</td>	
				<td>$jawaban</td>
				<td><div align='center'>
